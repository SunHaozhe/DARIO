...
def find_measures_arrays(net_orig, trainloader, dataload_info, device, measure_names=None, loss_fn=F.cross_entropy, head_mask=None, alphas_blocks=None):
        ...
        try:
            for measure_name in measure_names:
                if measure_name not in measure_values:
                    val = measures.calc_measure(measure_name, net_orig, head_mask, alphas_blocks, device, inputs, targets, loss_fn=loss_fn, split_data=ds)
    ...
    net_orig = net_orig.to(device).eval()
...
def find_measures(net_orig,                  # neural network
                  ...
                  head_mask=None,            # Self-activatation heads mask
                  alphas_blocks=None,        # Block-level architectural weights
    ...
    if measures_arr is None:
        measures_arr = find_measures_arrays(net_orig, dataloader, dataload_info, device, loss_fn=loss_fn, measure_names=measure_names, head_mask=head_mask, alphas_blocks=alphas_blocks)
